...
export const FetchIndiaNews = (props) => {
    ...
    const fetchNews = async () => {
        try {
            const response = await fetch(`https://newsapi.org/v2/top-headlines?country=us&category=${props.name}&pageSize=35&apiKey=${import.meta.env.VITE_NEWS_API_KEY}`)
    ...
    return (
        <MainContainer>
            ...
            <div className="newsCard">
                {news.slice(9, 15).map((article, index) =>
                    <NewsCard key={index} article={article} />
                )}
            </div>
            ...
            <TopStoriesContainer>
                ...
                <div>
                    <h4 style={{ marginLeft: '2rem' }}>Top Stories</h4>
                    {news.slice(18, 21).map((article, index) =>
                        <TopStories key={index} article={article} />)}
                </div>

            </TopStoriesContainer>
            <div className="newsCard">
                {news.slice(21, 35).map((article, index) =>
                    <NewsCard key={index} article={article} />
                )}
            </div>
        </MainContainer >
